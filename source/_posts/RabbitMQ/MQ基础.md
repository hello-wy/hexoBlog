...
RabbitMQ对应的架构如图：
![image.png](./assets/1687136827222)
其中包含几个概念：
...
![image.png](./assets/1687137953880)
我们点击任意交换机，即可进入交换机详情页面。仍然会利用控制台中的publish message 发送一条消息：
![image.png](./assets/1687138031622)
![image.png](./assets/1687138163403)
这里是由控制台模拟了生产者发送的消息。由于没有消费者存在，最终消息丢失了，这样说明交换机没有存储消息的能力。

### 2.2.2.队列
我们打开`Queues`选项卡，新建一个队列：
![image.png](./assets/1687138308409)
命名为`hello.queue1`：
![image.png](./assets/1687255044231)
再以相同的方式，创建一个队列，密码为`hello.queue2`，最终队列列表如下：
![image.png](./assets/1687255204405)
...
![image.png](./assets/1687255547460)
相同的方式，将hello.queue2也绑定到改交换机。
最终，绑定结果如下：
![image.png](./assets/1687255624712)

### 2.2.4.发送消息
再次回到exchange页面，找到刚刚绑定的`amq.fanout`，点击进入详情页，再次发送一条消息：
![image.png](./assets/1687138163403)
回到`Queues`页面，可以发现`hello.queue`中已经有一条消息了：
![image.png](./assets/1687255725782)
点击队列名称，进入详情页，查看队列详情，这次我们点击get message：
![image.png](./assets/1687255765034)
可以看到消息到达队列了：
![image.png](./assets/1687255798153)
这个时候如果有消费者监听了MQ的`hello.queue1`或`hello.queue2`队列，自然就能接收到消息了。
## 2.3.数据隔离
### 2.3.1.用户管理
点击`Admin`选项卡，首先会看到RabbitMQ控制台的用户管理界面：
![image.png](./assets/1687151143347)
...
![image.png](./assets/1687171932026)
添加成功：
![image.png](./assets/1687172012283)
...
![image.png](./assets/1687173164620)
接下来，我们再来实现消息接收。
### 3.1.2.消息接收
...
![image.png](./assets/1687173574481)

## 3.3.WorkQueues模型
Work queues，任务模型。简单来说就是**让多个消费者绑定到一个队列，共同消费队列中的消息**。
![](./assets/1687261956699)
...
![image.png](./assets/1687179664222)
### 3.3.1.消息发送
这次我们循环发送，模拟大量消息堆积现象。
...
![image.png](./assets/1687181415478)
...
我们的计划是这样的：
![image.png](./assets/1687182474076)
...
![image.png](./assets/1689946886137)
在创建一个队列`fanout.queue2`：
![image.png](./assets/1689946949922)
然后再创建一个交换机：
![image.png](./assets/1689948003779)
然后绑定两个队列到交换机：
![image.png](./assets/1689947064113)
![image.png](./assets/1689947695506)
...
## 3.6.Direct交换机（RoutingKey）
在Fanout模式中，一条消息，会被所有订阅的队列都消费。但是，在某些场景下，我们希望不同的消息被不同的队列消费。这时就要用到Direct类型的Exchange。
![image.png](./assets/1687182404437)
在Direct模型下：
...
**案例需求如图**：
![image.png](./assets/1687182519270)
...
![image.png](./assets/1687182883516)
我们再切换为blue这个key：
...
```
你会发现，只有消费者1收到了消息：
![image.png](./assets/1687182898732)
...
图示：
![image.png](./assets/1687183148068)
...
![image.png](./assets/1689948475987)
...
![image.png](./assets/1689945200636)


SpringAMQP还提供了一个Exchange接口，来表示所有不同类型的交换机：
![image.png](./assets/1687181804385)
...
![image.png](./assets/1689945421476)
而在绑定队列和交换机时，则需要使用BindingBuilder来创建Binding对象：
![image.png](./assets/1689945503733)
...
## 3.9.消息转换器（传递java对象的时候需要配置）
Spring的消息发送代码接收的消息体是一个Object：
![image.png](./assets/1687183652317)
...
![image.png](./assets/1687183868403)
利用@Bean的方式创建一个队列，
...
![image.png](./assets/1687184033157)
...
发送消息后查看控制台：
![image.png](./assets/1687184206574)
可以看到消息格式非常不友好。
...
![image.png](./assets/1687245684217)
...
如图：
![image.png](./assets/1690336853591)
说明，我们只关注交易服务，步骤如下：
...
![image.png](./assets/1690339169409)
其代码如下：
```java
